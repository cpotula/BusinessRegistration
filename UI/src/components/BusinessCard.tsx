...
export default function BusinessCard({ business }: { business: BusinessSummary }) {
  ...
  return (
    <Link
      to={`/b/${business.slug}`}
      className="group bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-lg hover:border-primary-200 transition-all duration-300 overflow-hidden"
    >
      <div className="p-5">
        <div className="flex items-start gap-4">
          {business.logoUrl ? (
            <img src={business.logoUrl} alt={business.name} className="w-14 h-14 rounded-xl object-cover bg-gray-100 shrink-0" />
          ) : (
          ...
          <div className="min-w-0">
            ...
            <span className="inline-block mt-1 px-2 py-0.5 rounded-full text-xs font-medium bg-primary-50 text-primary-700">
